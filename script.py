...
def train(model):
    ...
    try:
        for e in tqdm(range(EPOCHS)):
            ...
            plt.plot(store_qwk_epoch_loss[1:], label="Training Loss(QWK)")
            ...
            plt.plot(store_qwk_epoch_loss_val[1:], label="Validation Loss(QWK)")
        ...
        qwk_min_loss = min(store_qwk_epoch_loss_val)
        print("\nHighest accuracy of {} occured at {}...\nMinimum loss occured at {}... \nMinimum QWK loss of {} occured at {}".format(
            most_acc, store_epoch_acc_val.index(most_acc)+1, 
            store_epoch_loss_val.index(min_loss)+1), 
            qwk_min_loss, store_qwk_epoch_loss_val.index(qwk_min_loss)+1)
